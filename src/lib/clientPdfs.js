...
const NAVY = [30, 41, 59];
const BRAND_YELLOW = [253, 227, 73]; // #FDE349
const BRAND_GOLD = [219, 170, 30]; // #DBAA1E
...
async function startDoc(subtitle) {
  ...
  // Detalle de marca inspirado en el degradado amarillo/dorado del logo.
  doc.setFillColor(...BRAND_YELLOW);
  doc.rect(0, 90, pageWidth / 2, 2, "F");
  doc.setFillColor(...BRAND_GOLD);
  doc.rect(pageWidth / 2, 90, pageWidth / 2, 2, "F");
...
export async function generateRoutinePdf(
  ...
  const { doc, pageWidth, margin } = await startDoc(
...
function calcMealAverage(items, macroKeys) {
  ...
  /*
    Una comida puede tener:
      - alimentos comunes, sin número de opción
      - Opción 1
      - Opción 2
      - Opción 3...

    La regla es:
      1. sumar todos los alimentos de cada opción por separado
      2. calcular la media únicamente entre las opciones distintas
      3. añadir una sola vez los alimentos comunes, si existen
  ...
  */
  ...
  // Si no existen opciones explícitas, todos los alimentos pertenecen
  // a una única comida normal.
...
const macroLine = (m) =>
  `P ${formatPdfNumber(m.protein)}g  ·  C ${formatPdfNumber(
    m.carbs,
  )}g  ·  G ${formatPdfNumber(m.fat)}g`;
...
export async function generateDietPdf(client, dietEntries, preview = false) {
  ...
  const dayGroups = hasSpecificDays
    ? [1, 2, 3, 4, 5, 6, 7].map((d) => ({
        label: DAYS_MAP[d],
        items: dietEntries.filter(
          (e) => !e.dia_semana || Number(e.dia_semana) === d,
        ),
      }))
  ...
  dayGroups.forEach((group) => {
    ...
    MEALS.forEach((meal) => {
      const m = mealMacros(
        group.items.filter((e) => e.momento_dia === meal.id),
      );
    ...
    if (group.label) {
      ...
      doc.setFontSize(9.5);
      doc.text(
        `${formatPdfNumber(dayTotals.kcal)} kcal`,
        pageWidth - margin - 12,
        y + 3,
        { align: "right" },
      );
      ...
      doc.setTextColor(...BRAND_GOLD);
      doc.text(
        `Total del día: ${formatPdfNumber(dayTotals.kcal)} kcal`,
        margin,
        y,
      );
      y += 14;
    ...
    MEALS.forEach((meal) => {
      ...
      doc.setFontSize(9);
      doc.setTextColor(...GRAY);
      doc.text(`${formatPdfNumber(mm.kcal)} kcal`, pageWidth - margin, y, {
        align: "right",
      });
      y += 13;
      doc.setFontSize(8);
      ...
      const optionKeys = Object.keys(byOption).sort(
      ...
      optionKeys.forEach((optKey) => {
        ...
        byOption[optKey].forEach((it) => {
          ...
          doc.setTextColor(...NAVY);
          doc.text(
            `•  ${food.nombre} (${it.cantidad_g}${it.unidad || "g"})`,
            indent,
            y,
          );
          doc.setTextColor(...BRAND_GOLD);
